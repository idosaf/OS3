...
printf("start\n");
    for(int i=2; i<=60; i++) {
        ...
        for (int j = 0; j < 1; j++) {
          ...
          //  JobState state;
           // JobState last_state = {UNDEFINED_STAGE, 0};
            check1= (gettimeofday(&pre,NULL)==-1); // pre time
            JobHandle job = startMapReduceJob(client, inputVec, outputVec, i);
        ...
        int check2= (gettimeofday(&post,NULL)==-1); // post time
        if(check1+check2<0)  //check errors
            return-1;
        double gap = post.tv_usec-pre.tv_usec;
        double gap2= post.tv_sec-pre.tv_sec;
        gap=((gap/1000000)+gap2)/1;
        printf("%f\n\n",gap);
    }
